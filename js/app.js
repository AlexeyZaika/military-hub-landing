(() => {
    ...
    function headerSlider() {
        if (document.querySelector(".header__slider-body")) new Swiper(".header__slider-body", {
            ...
            effect: "fade",
            pagination: {
                el: ".header__slider-pagination-container",
                clickable: true
            }
        });
    }
    function catalogSlider() {
        if (document.querySelector(".catalog__slider-body")) new Swiper(".catalog__slider-body", {
            ...
            effect: "fade",
            pagination: {
                el: ".catalog__control-pagination",
                type: "fraction"
            },
            navigation: {
                nextEl: ".catalog__control-arrow-next",
                prevEl: ".catalog__control-arrow-prev"
            }
        });
    }
    function productSlider() {
        const productSliders = document.querySelectorAll(".catalog__product-slider");
        if (productSliders.length) for (const productSlider of productSliders) {
            const pagination = productSlider.nextElementSibling;
    ...
    function currentProductSlider() {
        if (document.querySelector(".product__slider-body")) new Swiper(".product__slider-body", {
            modules: [ Pagination, EffectFade ],
            observer: true,
            observeParents: true,
            slidesPerView: 1,
    ...
    function initializeSliderBg() {
        ...
        for (let i = 0; i < headerSlides.length; i++) {
            const headerSlide = headerSlides[i];
            const srcImage = headerSlide.getAttribute("data-image");
            headerSlide.style.backgroundImage = `url('${srcImage}')`;
        }
    }
    function initializePaginationImage() {
        const sliders = document.querySelectorAll(".swiper");
        for (const slider of sliders) {
            const paginations = slider.querySelectorAll(".swiper-pagination-bullet");
            const imageSlides = slider.querySelectorAll(".swiper-slide-image");
            if (paginations.length) for (let i = 0; i < paginations.length; i++) {
                const pagination = paginations[i];
                const imageSrc = imageSlides[i].getAttribute("src");
    ...
    function setPositionLineName() {
        const nameLines = document.querySelectorAll(".product-info__form-line-name.size");
        for (const nameLine of nameLines) {
            const labels = document.querySelectorAll(".product-info__form-label");
            const bodyLabels = document.querySelector(".product-info__form-body");
            for (const label of labels) label.addEventListener("click", (() => {
                const positionLabel = label.getBoundingClientRect().left - bodyLabels.getBoundingClientRect().left - 10;
                nameLine.style.left = positionLabel + "px";
            }));
        }
    }
    function playVideo() {
        const buttonPlay = document.querySelector(".product-info__button-play");
        buttonPlay.addEventListener("click", (e => {
            e.preventDefault();
            const slides = document.querySelectorAll(".product__slider-body .product__slider-slide");
            for (const slide of slides) if (slide.classList.contains("swiper-slide-active")) {
    ...
    function stopVideo() {
        ...
        const paginations = productSlider.querySelectorAll(".swiper-pagination-bullet");
        const videos = productSlider.querySelectorAll("video");
        for (const pagination of paginations) pagination.addEventListener("click", (() => {
            for (const video of videos) if (!video.paused) {
                video.pause();
                video.previousElementSibling.classList.remove("hide");
            }
        }));
        for (const video of videos) video.addEventListener("ended", (() => {
            video.previousElementSibling.classList.remove("hide");
        }));
    }
    window.addEventListener("load", (() => {
        initializeSliderBg();
        initializePaginationImage();
        playVideo();
        stopVideo();
